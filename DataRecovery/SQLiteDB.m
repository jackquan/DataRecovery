...
- (void) openDB {
    ...
    NSString * dbpath = databaseFileName;//[self getDBPath];
...
- (NSArray *)getSQLItems:(NSString *)sqlStr
{
    NSDictionary *row;
    NSMutableArray *idList = [[NSMutableArray alloc] init];
    for (row in [self getQuery:sqlStr]) 
    {
        [idList addObject:row];
    }
    return idList;
}
...
- (NSArray *)getSafaris
{
    NSDictionary *row;
    NSMutableArray *idList = [[NSMutableArray alloc] init];
    for (row in [self getQuery:SQLSafariLan]) 
    {
        if ([[row objectForKey:@"url"] isEqualToString:@""]) {
            continue;
        }
        [idList addObject:row];
    }
...
- (NSDictionary *) getRow: (NSNumber *) rowID {
    NSString * query = @"select b.First, b.Last, a.value from ABPerson b left join ABMultiValue a where a.record_id=b.rowid";
//[NSString stringWithFormat:@"select * from %@ where id = ?", tableName];
