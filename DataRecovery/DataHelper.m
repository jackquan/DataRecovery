...
+ (NSDictionary*) getFileListForPath:(NSString*)path {
    ...
    NSString *mbdxPath = [path stringByAppendingPathComponent:@"Manifest.mbdx"];
    if (![[NSFileManager defaultManager] fileExistsAtPath:mbdxPath]) {
        return mbdb;
    }
...
NSDictionary* process_mbdb_file(NSString* filename) {
    ...
    BOOL isMBDX = YES;
    NSString *mbdxPath = [filename.stringByDeletingLastPathComponent stringByAppendingPathComponent:@"Manifest.mbdx"];
    if (![[NSFileManager defaultManager] fileExistsAtPath:mbdxPath]) {
        isMBDX = NO;
    }
    ...
    while (offset < dataLength) {
        ...
        if (!isMBDX) {
            NSString *fullPath = [[fileinfo objectForKey:@"domain"] stringByAppendingFormat:@"-%@", [fileinfo objectForKey:@"filename"]];
            NSString *hashValue = [HashValue getHashValue:fullPath];
            [fileinfo setObject:hashValue forKey:@"fileID"];
        }
